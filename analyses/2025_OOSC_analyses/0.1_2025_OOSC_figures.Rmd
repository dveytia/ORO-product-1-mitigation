...
```{r read in policy documents}

## Read in and format policy matches

pols <- read.csv(here::here("data/derived-data/en-fr-de-sp_fullTextQueryMatches_allOROs_long.csv")) %>%
  ...
  mutate(
    component = "policy",
    variable_name = "Legislation documents (N)"
  )
...
pol_stackedBP <- ggplot(pols, aes(x=year, y=y, fill=oro_type))+
...
allComponentDat <- pubs %>%
  bind_rows(pols) %>%
  bind_rows(allDeployDat_scale) %>%
  mutate(
    component = factor(component, 
                       levels = c("publications","policy","deployment"), 
                       labels = c("Academic\n(N publications)", "Legislation\n(N documents)","Action\n(scaled metric)"))
  )


allComponentAlluvial <- ggplot(allComponentDat %>% filter(1980 <= year, year <= 2022), aes(x=year, y=y, alluvium=oro_type, fill=oro_type, colour=oro_type))+
  # geom_alluvium(alpha = 0.8, decreasing=FALSE)+
