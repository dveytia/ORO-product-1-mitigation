...
```{r remove - group oro networks conceptually and plot combined networks}
require(ggraph)
require(tidygraph)
...
plot_ggraph_network <- function(g, cluster_id, cluster_label, end_cap_mm = 10, end_cap_mult_factor=4) {
  ...
  ggraph(g_tidy, layout = "circle") +  # You can try "kk" or "circle" or "fr" for spacing
    ...
    geom_point(
      data = dot_positions,
      aes(x = dot_x, y = dot_y, size = 8*weight, #6*log1p(weight),
          alpha = relative_nsig),
      shape = 21,             
      fill = "#d01c8b",
      color="transparent",
    ...
    geom_node_label(aes(label = componentAES$label[match(name, componentAES$level)], fill = name), #repel = TRUE,
                   label.size = nodeTextSize, alpha=0.8, color = "white"
                   ) +
...
```{r v2 group oro networks conceptually and plot combined networks - simplified for clearer arrows}
require(ggraph)
require(tidygraph)
...
for (k in 1:n_clusters) {
  ...
  # E(g)$width <- log1p(E(g)$weight) * 1.5
  # E(g)$color <- scales::alpha(ifelse(E(g)$avg_correlation >= 0, "#4dac26", "#d01c8b"),edge_summary$relative_nsig)
  ## weight is avg ratio scaled from 0 to 1
...
plot_QNM <- function(k, nSims = 5000, weightSimulation = FALSE, uncertainThreshold=0.5){
  ...
  if(is.null(ModSim)){
    ...
    qnm_ggp <- ggplot(qp_results_df, 
      ...
      geom_text(aes(y=-Negative, label = paste0("-",Negative)), vjust=1)+
...
combined_patchwork <- wrap_plots(plotList, 
                                 design = plotLayout, heights=c(0.2,1),
                                 guides = "collect") +
    plot_annotation(tag_levels = 'a', tag_suffix = ")") &
