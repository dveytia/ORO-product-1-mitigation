...
# Publications: Mapping extent and distribution of evidence

## Distribution of publications

```{r barplot of n publications by outcome type}
...
Modify this to show uncertainty???
```{r plot publication timeseries -- MRE-Ocean is driving inflection}
...
```{r outcome type barplot vs oro timeseries  plots}
require(ggpubr)
...
## Investigate drivers of timeseries inflection


Do an area plot of the proportional contribution of different affiliations?

```{r calculate all author affiliations for all relevant references, eval=FALSE}
library(countrycode)
library(tidyr)
library(stringr)
require(dbplyr)
...
```{r plot publication timeseries -- by affiliation}
...
# oro_year_count_long <- predOroType %>%
#     filter(!(countries %in% affTypeTop10)) %>%
#     group_by(level, year) %>%
#     summarise(
...
## Overlay a lineplot of the interannual trend
...
# Correlation analysis (non-temporal)

Because I'm summarizing across years and comparing oros between each other, cannot include deployment because all the metrics are on different scales

MOdify to only look at primary research?
```{r fit glm between publication and all other components}
...
# keep each year as an indivdual data point or summarize across years? T
# This is easier for counts because I can sum, but for things like proportion
...
for(c in 1:length(components)){
  ...
  ggps[[c]] <- ggplot()+
    ...
    ggrepel::geom_text_repel(data = tmpDat, aes(x=publications, y = response_y, label = oro_type), force = 0.3,
                             size=2.5)+ 
...
```{r plot only a subset of the correlations for a main figure?}
...
ggpubr::ggarrange(
  ...
  labels = paste0(letters[1:length(selectedComponents)],") ", componentAES$label[match(selectedComponents, componentAES$level)]),
  label.x = 0, vjust =1.2, font.label = list(size = 10),
  align = "hv"
...
# Segway to TE -- Visualize timeseries of all components
...
```{r supplement of all alluvial components}
...
* Behrendt et al. 2019. RTransferEntropy—Quantifyinginformationflowbetweendifferent timeseriesusingeffective transfer entropy
...
# remove edge pairs whose calculations are inter-related (all the public metrics)
...
Directed Edges = significant NodeX → NodeY pairs with TE_pval < α (e.g. 0.05)
...
For each unique NodeX → NodeY, compute:
...
🔧 Description:
Use community detection or hierarchical clustering to identify subsystems (e.g., policy ↔ public interest loop).
...
```{r plot meta graph network and metric heatmap}
...
```{r build graphical model - Baysean network, eval = FALSE}
...
for (oro in oroTypes) {
  ...
  # 7. Loop over interventions on each node → observe effect on "deployment"
  # Since you’re simulating an intervention (e.g. P(deployment | do(policy = high))), what you want is the marginal distribution of deployment given the intervention.
...
Try 2: try at each oro level -- not convinced either because at a smaller scale some edges may not be detected. Could get a better representation of the network if we pool results...
```{r build qualitative network model for each oro, eval=FALSE}
...
Try 3: pool networks by oro type conceptually.
...
for (k in 1:n_clusters) {
  ...
  sig_te_edges <- TE_results %>%
    filter(TE_XCauseY == TRUE,!is.na(TE_ratio), oro_type %in% types_in_cluster) %>%
    group_by(NodeX, NodeY) %>%
    summarise(
      n_sig = n(),
      .groups = 'drop'
...
# Plot results
# For each ORO type
uniquePP <- unique(qp_concept_cluster_results_df$Press_perturb)
...
ggplot(qp_concept_cluster_results_df %>% filter(Positive>0), 
  ...
  geom_tile()+
  scale_fill_stepsn(name = stringr::str_wrap("Positive effect on action (% sims)", width=25), n.breaks=1, colours = viridis(5))+
  scale_x_discrete(
    breaks = componentAES$level[
  ...
  labs(y="ORO group",x="Press perturbation")+
  theme_bw()+
  theme(
    legend.position = "bottom",
    axis.text.x=element_text(angle=45, hjust=1)
...
Try 4: build qualitative model of meta network
```{r build qualitative network model for the meta network -- all oros}
# remotes::install_github("SWotherspoon/QPress")
library(QPress)
library(reshape2)
nSims <- 5000
...
ggplot(qp_results_df %>% filter(Positive>0), 
       aes(x=Press_perturb, y=stringr::str_wrap(cluster, width=10), fill = Positive))+
  geom_tile()+
...
### External drivers of deployment change points

```{r plot all deployment time series}
...
```{r qualitative CP attribution - external}
load(here::here("outputs/cutpointResults_mitigation.RData"))
...
# Whos conducting the field trials?
unique(oae_changeYears$`Leading Organization`) # "Vesta"   "GEOMAR"  "Limenet"

# Who are the startups?
unique(oae_changeYears$Company)
 unique(oae_changeYears$Company)
...
ggpubr::ggarrange(
  plotlist = list(mreOceanTime_ggp+theme(plot.margin = unit(c(1.2,1,1,0.5),"cm")), 
                  ccsStorTime_ggp, 
                  norCCS2014, 
                  effTime_ggp), 
  labels = paste0(letters[1:4], ") ",c("MRE-Ocean","CCS","CCS - Norway","Incr. Efficiency")), 
  label.x = 0, vjust =1.2, font.label = list(size = 10),
