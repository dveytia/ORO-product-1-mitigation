...
```{r Directed Causal Graph of TE results}
...
meta_links <- TE_results %>%
  filter(TE_pval < 0.05, !is.na(TE_ratio), TE_ratio > 0) %>%
...
Identify subsystems at the meta level
🔧 Description:
...
```{r Cluster nodes and plot the meta network}
clusters <- cluster_walktrap(meta_graph)


svg(file = here::here("figures/supplemental/meta_TE_network_diagraph.svg"),width = 8, height = 8)
plot(meta_graph, vertex.color = clusters$membership)
dev.off()
...
```{r Visualize the networks of each oro type cluster}
# Cut into 5 clusters
n_clusters <- 5
...
```{r cluster level networsk as ggplot}
library(ggraph)
library(tidygraph)
library(ggplot2)
...
for (oro in oroTypes) {
  ...
  for (node in all_nodes) {
    ...
    results_df <- results_df %>% bind_rows(temp)
...
sig_te_edges <- TE_results %>%
  filter(TE_pval <= 0.05, TE_XCauseY == TRUE,!is.na(TE_ratio)) %>%
  select(From=NodeX, To=NodeY) %>%
  mutate(
    Type = "P"
  )

sig_te_edges <- sig_te_edges[!duplicated(sig_te_edges),]
...
# Identify pairs
pairNodes <- sig_te_edges[!apply(sig_te_edges[,1:2], 1, is.unsorted), 1:2]
pairs <- rep(NA, length(nrow(sig_te_edges)))
for(p in 1:nrow(pairNodes)){
  for(i in 1:nrow(sig_te_edges)){
    ispair <- sum(sig_te_edges[i,1:2] %in% pairNodes[p,])==2
    if(ispair){
      pairs[i] <- p
    }else{
      next
...
sig_te_edges$Pair <- pairs  
sig_te_edges$Pair[is.na(sig_te_edges$Pair)] <- seq(p+1, p+sum(is.na(sig_te_edges$Pair)))
  


# sig_te_edges$Pair <- 1:nrow(sig_te_edges) # need to fix this to find pairs?
...
ModSim <- system.simulate(100, Mod) # simulate

# can I adjust the probability of selecting edges? 
# but here all the uncertain edges are given the same weight.... 
# need to figure out how to adjust this...maybe set a threshold?
...
for (node in all_nodes) {
  ...
  pooled_results_df <- pooled_results_df %>% bind_rows(temp)
...
for(oro in oroTypes){
  ...
  for(i in 1:length(components)){
    ...
    if(components[i] == "deployment" & oro == "CDR-OAE"){
    ...
    if(K == 0){
      ...
      pkIndKeep <- pkIndKeep[which(0 < diff(trends))] 
...
oroTypes <- unique(cutpoint_results$oro_type)
...
for(oro in oroTypes){
  ...
  for(cpid in unique(depCP$cp_id)){
    targetYear <- depCP$year[depCP$cp_id == cpid]
    attrCPs <- cutpoint_results %>%
      filter(component != "deployment", oro_type == oro, quantile == "50%",
             cp_id == cpid,
             year <= targetYear+2 & targetYear-4 <= year)
...
changePoint_ggp <- ggplot()+
  ...
  # Color scales
 scale_color_manual(name="Median change-point",
...
# ggsave(
#   here::here("figures/main/changePointTimeSeries.pdf"),
