...
# concatenates the number of views for 1 species for all languages studied + the total number of views
  ...
  for (i in 1:length(languages)){
    ...
    tmp <- get_all_page_views_lang(lan = languages[i], nam = name, start_date, end_date)
...
extract_langviews <- function(url) {
  # Read the HTML content from the URL
  page <- rvest::read_html(url)
  
  # Extract the table containing the language views data
  langviews_table <- page %>%
    rvest::html_node("table") %>%
    rvest::html_table(fill = TRUE,trim = TRUE)
  
  # Select and rename the 'Language' and 'Page title' columns
  langviews_df <- langviews_table %>%
    select(Language, `Page title`)
...
library(wdman)
rD <- rsDriver()
rD <- rsDriver(browser = "chrome", chromever = "133.0.6943.127",verbose = F,
               port= 4445L,#netstat::free_port()
...
langviews_df <- extract_langviews(paste0("https://pageviews.wmcloud.org/langviews/?project=en.wikipedia.org&platform=all-access&agent=user&range=latest-20&sort=views&direction=1&view=list&page=", c(gsub(" ","%20","Tidal power"))))

langview <- rvest::read_html(paste0("https://pageviews.wmcloud.org/langviews/?project=en.wikipedia.org&platform=all-access&agent=user&range=latest-20&sort=views&direction=1&view=list&page=", c(gsub(" ","%20","Tidal power")))) 

langview_text    <- rvest::html_nodes(x = langview, 'body #content #bodyContent #mw-content-text .mw-parser-output p') # get the body text of the article (if first time reading a html file, do it step by step to where you are going)
wiki_text    <- rvest::html_text(wiki_text,trim = TRUE)                                                    
```
...
for (i in 1:toend) {
  ...
  for (term in terms){
    ...
    lwiki <- list(ER=TRUE,wiki_scrap=NA, ER_count=0)
    while(lwiki$ER){
      tryCatch({
      ...
      error = function(e){
        # lwiki$ER <- TRUE
        cat("Error", lwiki$ER_count, " with the scraping, wait 60s and retry ... ","\n")
        # Sys.sleep(60)
...
ggplot(WTA_agregated, aes(x=oro_type, y=total_views)) +
  geom_col()+
  ...
  labs(y="Total wikipedia page views since 2015", x="")+
  theme_minimal()+
  theme(
