...
# Upset plot within each group 
require(ggupset)

termGroups <- unique(screen_tabs_term$Group)
...
for(i in 1:length(termGroups)){
  ...
  tempDat <- screen_tabs %>%
    filter(Group == termGroups[i], Match==1) %>%
    distinct(analysis_id, .keep_all = TRUE) 
    # unnest(cols = Term) %>%
    # mutate(TermMember =1) %>%
    # pivot_wider(names_from = Terms, values_from = TermMember, values_fill = list(TermMember = 0)) %>%
    # as.data.frame()
  ...
  upsetGGPs[[i]] <- ggplot(tempDat, aes(x=Term))+
    geom_bar()+
...
main_plot <- tidy_movies %>%
  distinct(title, year, length, .keep_all=TRUE) %>% head()
  ggplot(aes(x=Genres)) +
  geom_bar() +
  scale_x_upset(n_intersections = 20)
