...
*Key message*
Mitigation ORO publication effort is weighted towards marine renewable energy (ocean, located). Over time, we observe varying rates of increase, with inflection points – notably in MRE-Ocean occurring in 2001. This inspired this analysis: What are the drivers of this inflection point? 
...
*Skip the code and just run the chunk to include the figure at the end*
...
```{r plot all dimension timeseries together}
...
Step 2. Generalize these findings across OROs by creating a meta network across all the OROs. If an edge exists for an ORO, it is included in the meta network. Then, (only for ALL OROs network), use the meta network to build a qualitative network model, and simulate press perturbations to the different nodes and record the impact on action.
...
concept_ggraph_plots <- lapply(cluster_order, function(k) {
  plot_ggraph_network(concept_meta_graphs[[k]], k, concept_cluster_names2[[k]], dot_offset = 0.05)
})
...
```{r for mCDR and MRE and Efficiency compute QNM, eval=FALSE}
...
## Just networks and network metrics
plotList <- c(
  # MRE
...
ggsave(
  here::here("figures/main/conceptual_networks_heatmaps.pdf"),
  width=9, height=10, 
  plot=combined
)
...
## Include networks and network metrics AND QNM results
plotList <- c(
  # MRE
  concept_ggraph_plots[[4]],
  concept_metrics_heatmaps[[3]],
...
combined <- wrap_plots(plotList, ncol=3,
                       guides="collect", widths = c(1.7, 1,1))+
  plot_annotation(tag_levels = "a")
...
## Step 2.2: Create Network for ALL OROs from TE edges + QNM -- remove
...
meta_links$avg_ratio[meta_links$avg_ratio==Inf] <- max(
  meta_links$avg_ratio[is.finite(meta_links$avg_ratio)]
)
...
Build a qualitative network model -  Drawing upon the causal links identified using transfer entropy (ie. the meta-network in step 2), what does it imply about the drivers of ORO action? What levers can be pulled to affect change?
...
```{r plot all meta network and metrics and QNM barplots together, eval = FALSE}
...
knitr::include_graphics(here::here("figures/main/metaNetwork_networkMetricHeatmap_QNMbarplot.pdf"))
```
...
combined2 <- dist_component_ggp_bottom + combined + plot_annotation(tag_levels = "a")+plot_layout(widths = c(1.2,3), heights=c(1,0.2), design = plotLayout)
...
eff_allComponents <- ggps[[which(names(oro_groups)=="Other")]][which(oro_groups$Other == "Efficiency")]
