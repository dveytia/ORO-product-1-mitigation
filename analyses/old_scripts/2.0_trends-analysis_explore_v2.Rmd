...
```{r fit glm between publication and all other components, eval = FALSE}
...
for(c in 1:length(components)){
  ...
  if(components[c] == "public support agreement"){
    # If public support agreement, fit as a binomial model where success = public support, and failure = public opposition
    tmpDat <- allComponentDat_model %>%
...
tmpDat <- allComponentDat_model %>%
      ...
      group_by(oro_type, component) %>%
  mutate(
    trend = lm(y ~ year)$fitted.values,
    detrended = y - trend
  ) %>%
  ungroup()

ggplot(tmpDat, aes(x=year, y=detrended))+geom_col()+facet_grid(component~ oro_type , scales="free_y")


tmpDat <- tmpDat %>%
  group_by(oro_type, component) %>%
  summarise(
        y = sum(detrended, na.rm=T)
      )%>%
      pivot_wider(names_from = component, values_from = y, id_cols = c(oro_type)) %>%
      na.omit() %>%
  rename(y = `public interest`)
...
```{r proportion of all posts in that year}
components <- unique(allComponentDat_model$component) 
components <- components[!(components %in% c("publications","deployment","public opposition"))]
c=3
...
tmpDat <- allComponentDat_model %>%
      filter(component %in% c("publications", components[c])) %>%
  left_join(tmpDat_annualSum, by = c("year","component")) %>%
  mutate(
    y = y/total
  )
...
ggplot(tmpDat, aes(x=year, y=y))+geom_col()+facet_grid(component~ oro_type , scales="free_y")
...
tmpDat <- tmpDat %>%
  ...
  left_join(
    tmpDat_annualSum %>%
      filter(component == components[c]),
    by="year"
  )


ggplot(tmpDat, aes(x=publications, y=y, col = oro_type))+
  geom_point()+
  scale_color_manual(
...
# But we want to know, does increasing knowledge increase public interest? 
...
mod <- glm(y ~ publications:oro_type + year, 
               data = tmpDat %>% mutate(oro_type = as.factor(oro_type)),
               family = Gamma())
...
# But then overall public i


## Ratio of public interest to publications
...
tmpDat <- allComponentDat_model %>%
      ...
      na.omit()%>%
  left_join(
    tmpDat_annualSum %>% pivot_wider(names_from = component, values_from = total, id_cols = c(year)),
    by="year",
    suffix=c("","_total")
  )%>%
  mutate(
...
colnames(tmpDat) <- gsub(
  components[c], "y",
  colnames(tmpDat)
)
tmpDat$round_y = round(tmpDat$y)

ggplot(tmpDat, aes(x=publications, y=round_y, col = oro_type))+
  geom_point()+
  scale_color_manual(
    breaks = typeAES$level,
    values = typeAES$colour
...
mod <- glm(cbind(round_y, publications) ~ oro_type + year, 
               data = tmpDat,
               family = binomial())

mod <- glm(round_y ~ publications + oro_type + year, 
               data = tmpDat,
...
# mod <- glm(cbind(round(y), y_total) ~ prop_pub:oro_type + year, 
#                data = tmpDat,
#                family = binomial())
...
library(sjPlot)
plot_model(mod, type = "pred", terms = c("oro_type"), show.p = TRUE)

library(ggeffects)
tmpDat$ratio <- tmpDat$round_y / tmpDat$publications
pred <- ggpredict(mod, terms = "oro_type", condition = c(publications = 1))
pred <- merge(pred, coef_table, by.x = "x", by.y = "oro_type", all.x = TRUE)

# Prepare raw observed ratios
tmpDat$ratio <- tmpDat$round_y / tmpDat$publications
...
ggplot(pred, aes(x = x, y = predicted)) +
  ...
  geom_jitter(data = tmpDat,
              ...
              width = 0.2, height = 0, color = "red", alpha = 0.5) +
  # p-value labels above predicted points
  geom_text(aes(label = p_label, y = predicted + 0.5),
            size = 3, color = "black", vjust=0) +
  scale_y_continuous(limits = quantile(tmpDat$ratio, probs = c(0,0.9)))+
  labs(y = "Public interest posts per publication",
       x = "ORO type",
       title = "Predicted rates vs. observed per-publication ratios") +
  theme_minimal()

# Plot the residual effects of the 
...
```{r plot only a subset of the glm fits for a main figure, eval = FALSE}
