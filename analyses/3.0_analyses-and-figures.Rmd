...
## check how many policy documents in 2024 -- is it 0?
year_oro_summary_df %>% 
  filter(grepl("policy|legislation",component,ignore.case = TRUE), 2024 <= year) %>% 
  group_by(component, year) %>%
  summarise(
    total_N = sum(N, na.rm=T)
  ) %>%
  print()
#   component                  year total_N
#   <fct>                     <dbl>   <dbl>
# 1 Policy (N documents)       2024      10
# 2 Legislation (N documents)  2024      45
...
## Save just the proportion plot 

ggsave(
  here::here("figures/supplemental/distribution_by_component_proportion.pdf"),
  width = 5.5, height=6.5, 
  plot = year_component_prop_ggp + 
...
```{r plot all dimension timeseries --except proportion -- together in multipanel figure - legend on bottom}
## Plot only timeseries
...
## Save
ggsave(
  here::here("figures/main/distribution_by_component_alltimeseries_noProp.pdf"),
  width=9, height=7,
  plot = combined
)
```
...
## Drivers of spikes in deployment
```{r drivers of spikes in deployment for MRE and CCS}
## MRE-Ocean -----------------
...
mreOceanTime_ggp<- ggplot(MREOcean_changeYears %>%
  ...
  ), aes(year,y,label = label),size=3, inherit.aes = FALSE, hjust=0)+
  labs(
    x="Year",
...
length(unique(norwayCCS$site_name)) # 9 sites in Norway in 2014
sum(norwayCCS$`Storage capacity (Mt)`, na.rm=T) # 11115.9 Mt storage capacity in Norway in 2014
...
p_all <- wrap_plots(plotList, ncol = 2, widths = c(1,1), heights = c(1,1.5)) + 
...
# Save
ggsave(
  here::here("figures/supplemental/drivers_deployment_changes_MRE_CCS.pdf"),
  p_all,
  width = 10, height = 9
)
...
edges_dtw_df <- data.frame() # to save all dtw lag data
for(oro in oros){
  # oro <- "Efficiency" # for testing
  ...
  edgesResults <- data.frame(
    ...
    dtw_lag_mean=rep(NA, nrow(edges)),
    dtw_lag_sd=rep(NA, nrow(edges))
  )
  ...
  for(e in 1:nrow(edges)){
    ...
    X = oroDat$y_trans[oroDat$component == edges[e,1]] # cause
    Y = oroDat$y_trans[oroDat$component == edges[e,2]] # effect
    year = oroDat$year[oroDat$component == edges[e,1]] 
    ...
    edge_dat$year <- year[first_nonzero:last_nonzero]
    ...
    if(is.null(TE_out)){
      ...
      edgesResults$dtw_lag_mean[e] <- NA
      edgesResults$dtw_lag_sd[e] <- NA
    }else{
      ...
      # time series of optimal warping-path from DTW that is corrected by cross correlation. 
      # It is approximately that Y[t]=X[t-optIndexVec[t]])
      t = 1:length(edge_dat$X)
      DTW_lags = TE_out$follOut$optIndexVec[,1]
      edgesResults$dtw_lag_mean[e] <- mean(DTW_lags, na.rm=T)
      edgesResults$dtw_lag_sd[e] <- sd(DTW_lags, na.rm=T)
      ...
      ## Also save raw data 
      edges_dtw_df_tmp <- data.frame(
        oro_type = rep(oro, length(edge_dat$X)),
        year = edge_dat$year,
        xvar = edges[e,1],
        yvar = edges[e,2],
        x = edge_dat$X,
        y = edge_dat$Y,
        x_dtw_lag = edge_dat$X[t-DTW_lags],
        dtw_lag = -DTW_lags,
        pval = TE_out$pval,
        TE_ratio = TE_out$TEratio
      )

      edges_dtw_df <- bind_rows(edges_dtw_df, edges_dtw_df_tmp)
...
# clean
# rm(oroDat, nodes, edges, TE_out, edgesResults, edges_dtw_df_tmp)




save(TE_results, TE_results_summary, file = here::here("data/derived-data/TE_results.RData"))
save(edges_dtw_df, file = here::here("data/derived-data/TE_dtw_lag_data.RData"))
writexl::write_xlsx(edges_dtw_df, here::here("outputs/TE_dtw_lag_data.xlsx"))
...
load(here::here("data/derived-data/TE_dtw_lag_data.RData")) # edges_dtw_df
...
ggsave(
  here::here("figures/supplemental/VLTE_demonstration_plot2.pdf"),
  width = 4, height=3,
  plot = vlte_ggp
)
...
edge_col <- list(positive = "#4dac26", negative = "#d01c8b") # edge colors 
...
titleSize <- 15 #12
labelSize <- 3
# range of TE ratios across all edges to set consistent edge transparency scaling across clusters
TE_ratio_range <- range(TE_results$TE_ratio[TE_results$oro_type %in% concept_cluster_df$oro_type & !is.na(TE_results$TE_ratio) & is.finite(TE_results$TE_ratio)])
# set limits for edge transparency scaling (rescaling TE ratios to this range for plotting)
TE_scale_limits <- c(0.25, 1) # Adjust based on your TE_ratio range

# Data frame to hold summary stats for each edge in each cluster
concept_meta_stats <- data.frame()

# Loop through clusters and build meta-graph for each
for (k in 1:n_clusters) {
  ...
  cluster_name <- cluster_names$cluster_name[cluster_names$cluster == k]
  ...
  # Do we take the mean dtw lag per oro_type first, then aggregate across oro_types?
  # Or (shown below) I aggregate all dtw lag data across oro_types directly?
  # For now choose option 1, but keep this code (option 2) in case needed later

  # dtw_dat_te <- edges_dtw_df %>%
  #   filter(oro_type %in% types_in_cluster,
  #           xvar %in% selectedComponents,
  #           yvar %in% selectedComponents,
  #           pval <= 0.05, !is.na(TE_ratio), 0 < TE_ratio
  #   )%>%
  #   select(oro_type, NodeX = xvar, NodeY = yvar, dtw_lag) %>%
  #   group_by(NodeX, NodeY) %>%
  #   summarise(
  #     dtw_lag_mean = mean(dtw_lag, na.rm=T),
  #     dtw_lag_sd = sd(dtw_lag, na.rm=T),
  #     .groups="drop"
  #   )
  ...
  edge_summary <- cluster_te %>%
    ...
    summarise(
      ...
      avg_optDelay = mean(optDelay, na.rm=T),
      avg_dtw_lag = mean(dtw_lag_mean, na.rm=T),
      max_dtw_lag = max(dtw_lag_mean, na.rm=T),
  ...
  E(g)$color <- scales::alpha(
    ifelse(E(g)$avg_correlation >= 0, edge_col$positive, edge_col$negative),
    E(g)$weight
    )
  E(g)$sign <- edge_summary$sign
  E(g)$label <- ifelse(
    edge_summary$avg_dtw_lag > 0,
    paste0("lag = ", ceiling(edge_summary$avg_dtw_lag), ", ", edge_summary$oro_types),
    ""
  )
  ...
  edge_summary <- edge_summary %>%
    mutate(group_name = cluster_name) %>%
    relocate(group_name)
  concept_meta_stats <- concept_meta_stats %>%
    bind_rows(edge_summary)
  ...
  ggraph(g_tidy, layout = "circle") +  # You can try "kk" or "circle" or "fr" for spacing
    ...
    geom_edge_fan(
      aes(
        edge_width = width,
        edge_alpha = weight,
        # label = ifelse(cluster_label %in% c("CCS","Efficiency"), 
        #                paste0("lag = ", scales::number(avg_dtw_lag, accuracy = 1)),
        #                paste0("lag = ", scales::number(avg_dtw_lag, accuracy = 1), ", ", oro_types)
        #                ),
        # label = paste0("lag = ",ceiling(avg_dtw_lag), ", ", oro_types), 
        label = label,
        edge_color = color,
        filter = sign==1
      ),
    ...
    labs(title = paste(cluster_label), tag = paste0(letters[cluster_id],")")) +
    theme_void() +
    theme(
...
# Version with all clusters - CCS, mCDR, Efficiency, MRE
...
# Version with only MRE, Efficiency, and mCDR 
concept_ggraph_plots_selected <- lapply(c(4,3,2), function(k) {
  plot_ggraph_network(concept_meta_graphs[[k]], k, concept_cluster_names2[[k]], end_cap_mm = 10, end_cap_mult_factor=1.8) +
  labs(tag = "") # remove tag for selected version
})




# Create a legend for the concept_ggraph_plots with custom symbols
TE_scale_breaks <- pretty(TE_ratio_range, n = 3)


# Create dummy data for custom legend
legend_data <- data.frame(
  x = 1:2,
  y = 1:2,
  edge_type = factor(c('Positive Edge', 'Negative Edge'), 
                     levels = c('Positive Edge', 'Negative Edge'))
)

legend_plot <- ggplot(legend_data, aes(x = x, y = y, color = edge_type)) +
  geom_point(data = subset(legend_data, edge_type == 'Positive Edge'),
             size = 0.1, 
             key_glyph = draw_key_arrow) +
  geom_point(data = subset(legend_data, edge_type == 'Negative Edge'),
             size = 0.1, 
             key_glyph = draw_key_linepoint) +
  scale_color_manual(
    values = c('Positive Edge' = edge_col$positive, 'Negative Edge' = edge_col$negative),
    name = "Edge Type"
  ) +
  guides(
    color = guide_legend(
...
# Create separate alpha legend
alpha_legend_data <- data.frame(
  x = 1:3,
  y = 1:3,
  alpha_val = seq(TE_scale_limits[1], TE_scale_limits[2], length.out = 3)
)

alpha_legend_plot <- ggplot(alpha_legend_data, aes(x = x, y = y, alpha = alpha_val)) +
  geom_point(size = 4) +
  scale_alpha_continuous(
    range = TE_scale_limits,
    breaks = seq(TE_scale_limits[1], TE_scale_limits[2], length.out = length(TE_scale_breaks)),
    labels = TE_scale_breaks,
    name = 'Mean TE Ratio'
  ) +
  theme_void() +
  theme(legend.position = 'right')
...
edges_legend <- as_ggplot(ggpubr::get_legend(legend_plot))
alpha_legend <- as_ggplot(ggpubr::get_legend(alpha_legend_plot))

# Combine legends vertically
combined_legends <- edges_legend / alpha_legend
...
final_plot_selected <- wrap_plots(concept_ggraph_plots_selected, nrow = 2, widths = c(1,1)) +
  plot_annotation(tag_levels = "a", tag_suffix = ")") 

# add the legends to the left hand side of the combined plot without tags
final_plot_legends <- (combined_legends | final_plot) + 
  plot_layout(widths = c(0.2, 1))


final_plot_selected_legends <- (combined_legends | final_plot_selected) + 
  plot_layout(widths = c(0.2, 1))

# Show or save
ggsave(filename=here::here("figures/main/conceptual_cluster_networks.pdf"), final_plot_legends, width = 11, height = 8)
ggsave(filename=here::here("figures/main/conceptual_cluster_networks_selected.pdf"), final_plot_selected_legends, width = 11, height = 8)

## Save the excel of the edge summary stats for the meta networks
writexl::write_xlsx(concept_meta_stats, here::here("outputs/concept_cluster_meta_network_edge_summary_stats.xlsx"))
```
...
## Plot network metrics as a supplementary figure
plot_list <- list(
...
ggsave(
  here::here("figures/supplemental/network_metrics_heatmaps.pdf"),
  width = 10, height=10,
  plot = combined)
```
...
ggsave(
  here::here("figures/supplemental/QNM_barplots.pdf"),
  width=8, height=8,
  plot=combined
)
```
...
```{r plot networks and qnm results for conceptual clusters}
...
plotList <- c(
  # Networks - MRE, Efficiency, mCDR
  concept_ggraph_plots2[[4]], concept_ggraph_plots2[[3]], concept_ggraph_plots2[[2]],

  # QNM barplots - MRE, Efficiency, mCDR
  concept_qnm_barplots[[3]], concept_qnm_barplots[[2]], concept_qnm_barplots[[1]]
  
)

# Create column headers as text grobs
column_labels <- c("MRE", "Efficiency", "mCDR")
header_plots <- lapply(column_labels, function(label) {
  ggplot() + 
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, 
             fill = "grey85", color = "grey20", linewidth = 0.5) +
    annotate("text", x = 0.5, y = 0.5, label = label, size = 5, fontface = "bold") +
    theme_void() +
    theme(plot.margin = margin(0, 0, 0, 0))
})

# Combine headers with plots
combined <- wrap_plots(
  c(header_plots, plotList),
  ncol = 3, 
  byrow = TRUE,
  heights = c(0.1, 1.6, 1)
) +
...
# Add custom legends for edge type and TE ratio to the left of the combined plot
# Use wrap_elements on combined to preserve its tags
combined_plots_legends <- (wrap_elements(combined) | wrap_elements(combined_legends)) + 
  plot_layout(widths = c(1, 0.1))

ggsave(
  here::here("figures/main/conceptual_networks_qnm.pdf"),
  width=15, height=8, 
  plot=combined_plots_legends
)
...
ggps_prop_tagged <- mapply(
  function(p, tag) p + 
    ...
    theme(legend.position = "none",
          ...
          plot.tag = element_text(size = 13, hjust = 0, vjust = 1.2),
...
combined2 <- dist_component_ggp_bottom + combined + 
plot_annotation(tag_levels = "a", tag_suffix = ")")+
plot_layout(widths = c(1.3,3), heights=c(1,0.2), design = plotLayout) &
theme(
  legend.title.position = "top",
  legend.title = element_text(hjust = 0.5)
)
...
for(og in which(names(oro_groups)=="narrative")){
  ...
  for(o in 1:length(oro_groups[[og]])){
    ...
    facetLabels <- c("Action" = paste0("Action\n(", actionMetric, ")"),
                     ...
                     "Interest" = "Interest\nlog(N posts)",
                     "Support" = "Support\nlog(N positive posts x likes)"
                     # "Opposition" = "Opposition\nlog(N negative posts x likes)"
                     )
