...
```{r Gattuso 2018 - ocean iron fertilisation experiments km2}
...
field_trials_ggp2 <- field_trials_ggp2 +
  ...
  theme_bw()+
  theme(
    axis.line.y.left = element_line(color = Tol_muted[c(3)], linewidth = 1),
    axis.text.y.left = element_text(color = Tol_muted[c(3)]),
    axis.title.y.left = element_text(color = Tol_muted[c(3)]),
    axis.line.y.right = element_line(color = Tol_muted[c(6)], linewidth = 1),
    axis.text.y.right = element_text(color = Tol_muted[c(6)]),
...
## write DOIs to text file for exploration in Overton

```{r write DOIs to text file for exploration in Overton}
## connect to database
...
# Collect all articles predicted to be relevant for Mitigation, and get their dois
...
pred_OROmitigation <- reshape2::melt(pred_OROmitigation, id.vars = "analysis_id", variable.name = "ORO_type",
...
# remove any duplicates where a single paper is predicted relevant for multiple ORO types
pred_OROmitigation <- pred_OROmitigation[!duplicated(pred_OROmitigation$analysis_id),]


# Format dois consistently
dois <- gsub("https://doi.org/", "", pred_OROmitigation$doi)
length(dois) # 34704 DOIs in total

# # Write to .txt. file
# file_con <- file(here::here("outputs/predictedRelevantMitigationOroDois.txt"))
# writeLines(dois, file_con)
# close(file_con)
```


## Explore FAOLEX database

Downloaded data by selecting 'sea' domain, and then keywords: 'climate change' or 'renewable energy'
Note the pdf with the same name has links to the original document
```{r}
fdf <- readxl::read_excel(here::here("data/raw-data/external/FAOLEX_sea_climate-change_renewable-energy.xlsx"))
colnames(fdf)[1] <- "faolexID"

fdf <- fdf %>%
  filter(!(`Primary Subject` %in% c("Land and soil",
                                  "Land and soil; Wild species and ecosystems",
                                  "Environment; Land and soil",
                                  "Land and soil; National development",
                                  "Forestry; Wild species and ecosystems",
                                  "Environment; Forestry; Water; Wild species and ecosystems; Climate change",
                                  "Agricultural and rural development",
                                  "Agricultural and rural development; Wild species and ecosystems",
                                  "Agricultural and rural development; Environment; National development",
                                 "Gender; Climate change",
                                 "Gender",
                                 "Agricultural and rural development; Private sector / entrepreneurship")))


## Plot on map
fdf$iso_code = countrycode::countrycode(fdf$Jurisdiction, "country.name", "iso3c")
...
# join'
join1 <- merge.data.frame(world_sf, fdf %>% group_by(Jurisdiction) %>% summarise(n_docs = n()), by.x = "name_long", by.y = "Jurisdiction", all.x = TRUE, all.y = FALSE)
join2 <- merge.data.frame(join1 %>% filter(is.na(n_docs)) %>% dplyr::select(-c(n_docs)), 
                          fdf %>% group_by(Jurisdiction) %>% summarise(n_docs = n()), by.x = "name_en", by.y = "Jurisdiction", all.x = TRUE, all.y = FALSE)

world_join <- rbind(
  join1 %>% filter(!is.na(n_docs)),
  join2
) %>%
...
# base plot
policyMap <- ggplot() + 
  geom_sf(data = world_sf, fill="transparent", size = 0.2) +
  geom_sf(data = world_join, aes(fill = n_docs), size = 0.2) +
  scale_fill_viridis_c()+ 
  theme_minimal() +
  coord_sf(crs = crs_robin)+
  labs(title = "Number FAOLEX documents",
       fill = "N Docs",
       caption = paste("Documents retrieved by search for 'climate change' and 'renewable energy' in the 'sea' domain. \n Documents can be polcies, legislation, regulation, international agreement, constitution\n Documents spatially matched =", sum(world_join$n_docs, na.rm=TRUE), "/", nrow(fdf)))
...
require(tidyr)
fdf_area <- fdf %>%
  mutate(Date = as.Date(as.numeric(`Date of text`), origin = as.Date("1900-01-01")),
         year = format.Date(Date, "%Y")) %>%
  filter(as.Date("2000-01-01") <= Date) %>%
  dplyr::select(`Type of text`, year) %>%
  na.omit %>%
  group_by(`Type of text`, year) %>%
  summarise(n = n()) %>%
  ungroup() %>%
  tidyr::complete(`Type of text`, year, fill = list(n=0)) %>%
  arrange(`Type of text`, year) %>%
  group_by(`Type of text`) %>%
  mutate(cumCount = cumsum(n)) 

ggplot(fdf_area, aes(x=as.Date(paste0(year, "-01-01")), y = cumCount, fill = `Type of text`)) + 
  geom_area()+
  scale_x_date(limits = c(as.Date("2000-01-01"), as.Date("2024-12-31")))+
  scale_fill_discrete()+
  labs(x="Date",y = "Cumulative document count")+
  theme_bw()+
  theme(
    legend.position = "bottom"
  )
