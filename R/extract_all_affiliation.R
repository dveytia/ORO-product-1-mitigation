...
#' Extract The Affiliation Of The First Author if French
#'
#' @param data a dataframe with at least two column called analysis_id and affiliation
#' @param countries_ls a dataframe with all existing countries (load sql-pays.csv file)
#'
...
#'
#'
extract_1stA_France <- function(data){

  aff_1stA <- data |> 
    filter(!is.na(affiliation)) |> 
    mutate(affiliation = stringr::str_replace_all(affiliation, c(
                                                                 "\\,FRANCE\\."                = ", France.",
                                                                 "\\, FRANCE\\."               = ", France.",
                                                                 "French Guiana"               = "France"))) |> 
    dplyr::mutate(country_aff = stringr::str_extract(affiliation, 
                                                     "France|French" #paste(countries_ls$name_en, collapse = "|")
                                                     ))
  
  oroAffiliations_country1stA <- aff_1stA |> 
    dplyr::filter(!is.na(country_aff))
  
  
  return(oroAffiliations_country1stA)
