...
"""
This version fits for all OROs together. 
"""
...
codedVariablesTxt1 = f'{dataFolder}/data/articleAnswers_formatted_2025-03-17_v2.txt'
codedVariablesTxt2 = f'{dataFolder}/data/articleAnswers_notMRE_formatted_2025-05-26_v2.txt' # file with coded variables
...
# create a binary variable for ecosystem
df[targetVar] = df[[x for x in df.columns if 'ecosystem' in x]].apply(lambda x: 1 if any(x)==1 else 0, axis=1)


## summarize
df.loc[df[targetVar]==1,[x for x in df.columns if 'oro_type' in x]].sum()
