...
# For testing:
df = df.groupby([targetVar, 'random_sample']).sample(n=5, random_state=1)
...
# This step is to make the script applicable for binary and multi label
df['labels'] = list(df[targetVar].values)
...
for k, (train, test) in enumerate(cv.split(df, random_sample=df['random_sample'].values)): # enumerate(outer_cv):    
    ...
    # # Un-nest data and convert labels to numeric -- no need when binary label
    ...
    eps = evaluate_preds(y_test, preds)
    eps["fold"] = k
