...
"""
Maybe for this version change the prediction boundary so that it's all the articles NOT any of the other mitigation types? Not just MRE?
STILL NEED TO MODIFY SCRIPT
"""
...
df = df[df[f'oro_type.MRE-Located']==0]
df = df[df[f'oro_type.MRE-Ocean']==0]
...
## Instead, modify KFoldRandom so all random articles are in every fold
outer_cv = KFoldRandom(n_folds, df.index, df[df['random_sample']!=1].index, discard=False)

# for k, (train_idx, test_idx) in enumerate(outer_cv):
#     print(f"\nFold {i+1}")
#     print("Train:", df.iloc[train_idx][targetVar].sum())
#     print("Test :", df.iloc[test_idx][targetVar].sum())


cv = KFoldWithFixedTest(n_splits=n_folds, shuffle=False)
# for i, (train_idx, test_idx) in enumerate(cv.split(df, random_sample=df['random_sample'].values)):
#     print(f"\nFold {i+1}")
#     print("Train:", df.iloc[train_idx][targetVar].sum())
#     print("Test :", df.iloc[test_idx][targetVar].sum())
...
############################## Run models ################################
######################## Change file path (x3) ###########################
for k, (train, test) in enumerate(cv.split(df, random_sample=df['random_sample'].values)): # enumerate(outer_cv):    
    if k!=rank_j:
        continue
    try:
        pr = param_space[0]
