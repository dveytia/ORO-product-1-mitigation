...
tempDf.loc[tempDf['primary_research - mean_prediction']>=0.5,[x for x in tempDf.columns if 'oro_type' in x]].apply(lambda x: np.where(x>0.5,1,0)).sum()

"""
No ORO types disappeared or were penalized disproportionately, so predicting across all ORO types is ok. Don't need to nest predictions by ORO type
oro_type.MRE-Located - mean_prediction    15619
oro_type.MRE-Ocean - mean_prediction      26497
oro_type.CCS - mean_prediction             2162
oro_type.CDR-BC - mean_prediction          2728
oro_type.MRE-Bio - mean_prediction          367
oro_type.Efficiency - mean_prediction      4877
oro_type.CDR-OAE - mean_prediction          235
oro_type.CDR-BioPump - mean_prediction      438
